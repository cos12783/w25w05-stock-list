...
const TickerCard = ({ ticker }) => {
  ...
  if (error) {
    return (
      ...
      text-red-700 px-4 py-3 rounded relative w-80 mx-auto">
  ...
  return (
    ...
    transform transition duration-500 hover:scale-105">
      <div className="flex justify-between items-center mb-4">
        ...
        text-gray-800">{name}</h2>
        
        <div className="text-sm font-semibold 
        text-gray-500">{ticker}</div>
      ...
      <div className={`text-4xl font-extrabold mb-2 
        ...
        {currencyMarker}{isKoreanStock? 
        currentPrice.toLocaleString() : currentPrice.toFixed(2)}
      </div>

      <div className={`text-base font-semibold 
        ${isPositive ? 'text-green-700' : 'text-red-700'}`}>
        {isPositive ? '▲' : '▼'} {isKoreanStock? 
        priceChange.toLocaleString() : priceChange.toFixed(2)}
      </div>
      
      <div className="mt-4 text-sm text-gray-500">
        전일 종가: {currencyMarker}{isKoreanStock? 
        previousClose.toLocaleString() : previousClose.toFixed(2)}
      </div>
    </div>
    </a>
  )
